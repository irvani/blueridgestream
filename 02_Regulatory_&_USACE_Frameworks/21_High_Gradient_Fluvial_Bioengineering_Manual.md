...
## 1. Introduction to High-Gradient Mountain Streams & Natural Channel Design (NCD)
...
```
   ...
   (Q_bkf)  |   Colluvial Slopes  \           /   Colluvial Slopes  |
            ...
            \_______________________/   |   \_______________________/
                      [Bedrock Step / Keystone Structure]
```
...
#### 1. Average Boundary Shear Stress ($\tau$)
The spatial average shear stress exerted by flowing water on the channel bed and banks is defined as:

$$\tau = \gamma R_h S$$

Where:
*   $\tau$ = Average boundary shear stress ($\text{lb/ft}^2$ or $\text{N/m}^2$)
*   $\gamma$ = Unit weight of water ($62.4 \text{ lb/ft}^3$ or $9,810 \text{ N/m}^3$)
...
*   $\rho_w$ = Density of water ($1.94 \text{ slugs/ft}^3$ or $1,000 \text{ kg/m}^3$)
*   $g$ = Gravitational acceleration ($32.2 \text{ ft/s}^2$ or $9.81 \text{ m/s}^2$)

#### 3. Critical Shear Stress ($\tau_c$) for Particle Entrainment
...
Where:
*   $\tau_c$ = Critical shear stress ($\text{N/m}^2$ or $\text{lb/ft}^2$)
*   $\theta_c$ = Dimensionless critical Shields parameter (dimensionless, typically $0.045$ to $0.060$ for gravel-bed channels; reduced to $0.030$ to $0.035$ for steep slopes due to gravity assistance on slopes $> 0.02$)
*   $\rho_s$ = Density of sediment particles (typically $2,650 \text{ kg/m}^3$ or $5.14 \text{ slugs/ft}^3$ for quartz/granite rock)
*   $\rho_w$ = Density of water ($1,000 \text{ kg/m}^3$ or $1.94 \text{ slugs/ft}^3$)
...
```
                  RHODODENDRON MAXIMUM STRUCTURAL MATRIX
                  
                     [Interlocking Evergreen Canopy]
                              \   |   /
                               \  |  /
                                _\|/_
                               /     \
                              |       | [Multi-stem Trunk Cluster]
                               \  |  /
                                \ | /
                                 \|/
         ========================/ \======================= [Soil Surface]
                   ...
                   / /    /    /     \    \    \ \
                 / /    /     /       \     \    \ \
               [Dense Shallow-Spreading Fibrous Root Mat] (0.3 - 0.9m Depth)
```
...
$$R_{rb} = \alpha_{soil} \cdot \left( \sum_{i=1}^{n} D_{i} \right) \cdot \sqrt[3]{\frac{\text{LAI}}{\text{LAI}_{ref}}} + L_{min}$$

Where:
*   $R_{rb}$ = Minimum root-ball extraction radius (in inches) measured from the geometric center of the root mass cluster.
*   $D_i$ = Basal diameter of the $i$-th mature stem within the cluster (in inches), measured $6 \text{ inches}$ above the root crown.
*   $\alpha_{soil}$ = Soil texture multiplier coefficient:
    *   $\alpha_{soil} = 1.2$ for tight, cohesive silt-clays.
    *   $\alpha_{soil} = 1.5$ for gravelly sandy loams (standard forest floor soils in Blue Ridge).
    *   $\alpha_{soil} = 1.8$ for loose, non-cohesive alluvial sand-gravel bed mixtures.
*   $\text{LAI}$ = Measured Leaf Area Index of the transplant cluster canopy ($\text{m}^2/\text{m}^2$).
*   $\text{LAI}_{ref}$ = Reference Leaf Area Index for wild mature *Rhododendron* ($5.0 \text{ m}^2/\text{m}^2$).
*   $L_{min}$ = Absolute mechanical minimum threshold (set strictly to $12 \text{ inches}$ to ensure structural mass integrity).
...
$$V_{rb} = \frac{2}{3} \pi R_{rb}^2 H_{rb}$$
...
$$W_{rb} = V_{rb} \cdot \rho_{soil\_wet}$$

Where $\rho_{soil\_wet}$ is the wet bulk density of the soil (typically $115 \text{ lb/ft}^3$ to $130 \text{ lb/ft}^3$ depending on gravel content).
...
The contribution of plant roots to stream bank soil stability is modeled as a dynamic increase in soil shear strength. The shear strength of root-reinforced soil ($s$) is calculated using the classic Mohr-Coulomb equation, augmented by the Wu-Waldron Model (WWM) root-cohesion term ($\Delta S_r$):

$$s = c' + \sigma' \tan \phi' + \Delta S_r$$

Where:
*   $s$ = Total shear strength of the root-reinforced soil ($\text{lb/ft}^2$ or $\text{kPa}$)
*   $c'$ = Effective soil cohesion without roots ($\text{lb/ft}^2$ or $\text{kPa}$)
*   $\sigma'$ = Effective normal stress on the shear plane ($\text{lb/ft}^2$ or $\text{kPa}$), calculated as $\sigma' = (\sigma - u)$ where $u$ is pore water pressure
*   $\phi'$ = Effective internal friction angle of the soil (degrees)
*   $\Delta S_r$ = Root-cohesion reinforcement factor ($\text{lb/ft}^2$ or $\text{kPa}$)

#### 1. Calculating Root Cohesion ($\Delta S_r$): The Wu-Waldron Model (WWM)
The mechanical reinforcement term $\Delta S_r$ assumes that roots act as elastic fibers that cross a potential failure shear plane. Under tension, the roots convert shear strain into tensile force:

$$\Delta S_r = k' \cdot \sum_{j=1}^{m} \left[ T_{j} \cdot \left( \frac{A_{rj}}{A} \right) \right]$$

Which simplifies in engineering design to:

$$\Delta S_r = k' \cdot T_{root} \cdot \text{RAR}$$

Where:
*   $T_{root}$ = Average tensile strength of the root fibers ($\text{lb/in}^2$ or $\text{MPa}$)
*   $\text{RAR}$ = Root Area Ratio (dimensionless), calculated as $\text{RAR} = A_{roots} / A_{total}$ (the ratio of the total cross-sectional area of roots crossing the shear plane ($A_{roots}$) to the total area of the shear plane ($A_{total}$))
*   $k'$ = Dimensionless coefficient accounting for root orientation relative to the shear plane at failure:
    ...
    Where $\theta_r$ is the angle of root shear distortion (typically assumed to be $48^\circ$ to $52^\circ$ at peak soil strain). Empirical testing across multiple fluvial soils establishes that $k'$ ranges from $1.15$ to $1.30$. A conservative value of **$1.2$** is standard in fluvial engineering designs.
...
| Soil Depth Zone ($z$, cm) | Root Area Ratio (RAR) | Average Root Tensile Strength ($T_{root}$, MPa) | Resulting Soil Cohesion Increase ($\Delta S_r$, kPa) | Resulting Soil Cohesion Increase ($\Delta S_r$, $\text{lb/ft}^2$) |
| :--- | :--- | :--- | :--- | :--- |
...
*Note: Calculations use $k' = 1.2$. Note how the massive RAR in the top $30\text{ cm}$ provides a major structural armor layer against hydraulic shear erosion.*
...
```
                  CROSS-SECTIONAL STRUCTURAL PROFILE: 3-TIER GRSL
                  
                                              [Mature Rhododendron Canopy]
                                                      \   |   /
                                                    === _\|/_ === [Top Bank]
  [Tier 3 Lift: 1.0 ft]   ==========================|=(*_*_*)=|==
                          |   Compacted Soil Silt-Loam |     / \ | [Root Ball]
                          |   Woven Coir Fabric (700g) |    /   \| 
  [Tier 2 Lift: 1.2 ft]   |============================|=========|===
                          |                            |  Roots  | 
                          |   Compacted Gravelly Loam  |=========|=== [Live Stake Matrix]
                          |                            |         | 
  [Tier 1 Lift: 1.5 ft]   |============================|=========|===
                          |                                      | [Structural Root Wad]
                          |   Compacted Bank Run Gravel          |   (Primary Flow Deflector)
  ========================|=========================             | 
  [Stream Bed]  _.._..__..|.._.._..__.._.._.._..__.._ \__________/
                          \__________________________/
                               [Toe Rock Foundation]
```

#### Lift Assembly Specifications:
1.  **Toe Rock Foundation**: Non-acid producing granite boulders ($d_{50} \ge 2.5 \text{ ft}$) are placed along the sub-surface footing, keyed a minimum of $2.0 \text{ ft}$ below the design scour depth of the pool.
2.  **Tier 1 Lift (Basal Lift)**:
    *   **Thickness**: $1.5 \text{ ft}$ compacted thickness.
    *   **Fill Material**: Compacted bank-run gravel/sand mixture (similar to bed material) to ensure rapid drainage.
    *   **Reinforcement**: Wrapped in $900\text{ g/m}^2$ high-tensile coir fabric (double wrapped on the outer face).
3.  **Tier 2 Lift (Transition Lift)**:
    *   **Thickness**: $1.2 \text{ ft}$ compacted thickness.
    *   **Fill Material**: Gravelly sandy loam mixed with organic compost ($4:1$ ratio).
    *   **Reinforcement**: Wrapped in $700\text{ g/m}^2$ coir fabric. Underlain by a high-strength biaxial polymer geogrid (tensile strength $\ge 20 \text{ kN/m}$) extending $6.0 \text{ ft}$ back into the parent bank slope.
    *   **Biocomponents**: Living branch-packing arrays (composed of $70\%$ *Alnus serrulata* (Tag Alder) and $30\%$ *Cornus amomum* (Silky Dogwood)) are placed horizontally between Tier 1 and Tier 2 at an angle of $15^\circ$ to the horizontal.
4.  **Tier 3 Lift (Upper Riparian Lift)**:
...
```
              GEOTECHNICAL FORCE BALANCE ON GRSL SYSTEM
              
                     Parent Soil Bank
                     .--------------.
                     |              |
                     |  Active Earth| --> [Soil Lifts (W_GRSL)]
                     |  Pressure    |     |
                     |  (P_active)  |     v (Gravity)
                     |       ------->     |
                     |              |     |
                     '--------------'     v
             _____________________________|_______________ [Failure Plane]
             <--------------------------------------------
                       Resisting Frictional Force (F_resist)
```
...
$$FS_{sliding} = \frac{\sum F_{\text{resisting}}}{\sum F_{\text{driving}}} = \frac{\left[ W_{GRSL} \cdot \cos \beta + F_{anchor} \right] \tan \delta + c_b' \cdot L_b}{P_{active}}$$

Where:
*   $W_{GRSL}$ = Total weight of the compacted soil lifts and plant root-balls per unit linear foot ($\text{lb/ft}$):
    
    $$W_{GRSL} = \sum_{j=1}^{3} \left( H_j \cdot B_j \cdot \rho_{soil\_wet} \right) + W_{rb}$$
    
    *(With height $H$, base width $B$, and soil bulk density $\rho_{soil\_wet}$ for each lift $j$)*
*   $\beta$ = Slope angle of the failure plane (assumed horizontal, $0^\circ$)
*   $F_{anchor}$ = Total mechanical resistance provided by the structural root wads and wooden anchor stakes driven through the failure plane ($\text{lb/ft}$)
*   $\delta$ = Interface friction angle between the coir fabric and the soil failure plane (typically $\delta = 0.9 \cdot \phi'$)
*   $c_b'$ = Cohesion of the bank soil at the failure plane interface ($\text{lb/ft}^2$)
*   $L_b$ = Length of the horizontal base failure plane ($B_{\text{Tier 1}} \approx 6.0 \text{ ft}$)
*   $P_{active}$ = Lateral active earth driving force ($\text{lb/ft}$) calculated via Rankine's earth pressure theory:
    
    $$P_{active} = \frac{1}{2} K_a \gamma_{bank} H_{total}^2$$
    
    Where the active pressure coefficient $K_a = \tan^2(45^\circ - \phi'/2)$, $\gamma_{bank}$ is the unit weight of the bank soil, and $H_{total}$ is the total height of the 3-tiered system ($1.5 + 1.2 + 1.0 = 3.7 \text{ ft}$).

*Safety Criterion*: $FS_{sliding}$ must be **$\ge 1.50$** under dry conditions and **$\ge 1.30$** under fully saturated drawdown conditions.

#### 2. Factor of Safety against Overturning ($FS_{overturning}$)
The system must resist rotational tipping around the outer toe of the bottom lift:

$$FS_{overturning} = \frac{\sum M_{\text{resisting}}}{\sum M_{\text{overturning}}} = \frac{W_{GRSL} \cdot \left( \frac{B_1}{2} \right) + F_{anchor} \cdot X_{anchor}}{P_{active} \cdot \left( \frac{H_{total}}{3} \right)}$$

Where $X_{anchor}$ is the horizontal lever arm distance from the toe to the anchor stakes.

*Safety Criterion*: $FS_{overturning}$ must be **$\ge 2.00$** under all conditions.
...
```
                     GRID PATTERN FOR LIVE-STAKING
                     
                       Colleague Distance (1.5 ft)
                        |<------ 1.5' ------>|
                     ---O--------------------O---
                     \  |  \                 \  |  \
                      \ |   \                 \ |   \
              ...
              O = Alnus serrulata     X = Cornus amomum
```
...
*   **Grid Density**: Stakes must be driven at a spacing of **$1.5 \text{ ft} \times 1.5 \text{ ft}$** on center in a staggered diamond pattern.
...
*   **Embedment**: The main stem of the root wad (the trunk) is driven horizontally a minimum of $8.0 \text{ ft}$ into the parent bank.
*   **Pinning**: The trunk is pinned down using a minimum of two vertical wooden piles ($6 \text{ inch}$ diameter, driven $4.0 \text{ ft}$ into the subbed soil) and secured with high-strength galvanized steel cable ($1/4\text{-inch}$ utility grade).
...
To maintain stream temperatures strictly below the critical spawning threshold of **$65^\circ\text{F}$ ($18.3^\circ\text{C}$)** and ensure optimal spawning temperatures of **$< 58^\circ\text{F}$ ($14.4^\circ\text{C}$)**, bioengineered banks must provide complete thermal shielding through calculated canopy shade design.
...
The net solar radiation reaching the stream surface ($\Phi_{\text{net\_solar}}$, $\text{W/m}^2$) under an interlocking mature *Rhododendron* canopy is calculated using the following adaptation of the Beer-Lambert law:

$$\Phi_{\text{net\_solar}} = I_0 \cdot (1 - \alpha_{\text{canopy}}) \cdot e^{-\text{LAI} \cdot k_{\text{ext}}} + \Phi_{\text{diffuse}}$$

Where:
*   $I_0$ = Daily peak incident global solar radiation above the canopy ($\text{W/m}^2$), typically peaking at $950 \text{ W/m}^2$ during summer solstice in North Georgia.
*   $\alpha_{\text{canopy}}$ = Albedo of the *Rhododendron* canopy (dimensionless, typically $0.12$ to $0.15$ for deep green glossy broadleaf evergreen leaves).
*   $\text{LAI}$ = Leaf Area Index of the *Rhododendron maximum* canopy ($\text{m}^2/\text{m}^2$). Highly dense wild stands exhibit an $\text{LAI}$ ranging from **$4.5$ to $6.2$**.
*   $k_{\text{ext}}$ = Canopy light extinction coefficient (dimensionless, typically $0.70$ for broadleaf evergreen shrub structures).
*   $\Phi_{\text{diffuse}}$ = Sky diffuse radiation component penetrating canopy gaps (typically assumed to be a constant $15 \text{ W/m}^2$ under dense forest cover).
...
$$\Phi_{\text{net\_solar\_degraded}} = 950 \cdot (1 - 0.05) \cdot e^{0} + 0 = \mathbf{902.5 \text{ W/m}^2}$$
...
$$\Phi_{\text{net\_solar\_shaded}} = 950 \cdot (1 - 0.14) \cdot e^{-5.2 \cdot 0.70} + 15$$

$$\Phi_{\text{net\_solar\_shaded}} = 817 \cdot e^{-3.64} + 15 = 817 \cdot 0.0262 + 15 = 21.4 + 15 = \mathbf{36.4 \text{ W/m}^2}$$
...
$$\frac{dT}{dx} = \frac{W \cdot \Phi_{\text{total\_heat}}}{\rho_w \cdot C_p \cdot Q}$$

Where:
*   $dT/dx$ = Water temperature change per unit channel length ($^\circ\text{C/m}$)
*   $W$ = Stream water surface width (m), typically $4.5 \text{ m}$ for mountain pools.
*   $\Phi_{\text{total\_heat}}$ = Total net heat flux ($\text{W/m}^2$), calculated as the sum of solar, longwave, evaporative, convective, and hyporheic heat fluxes:
    
    $$\Phi_{\text{total\_heat}} = \Phi_{\text{net\_solar}} + \Phi_{\text{longwave}} - \Phi_{\text{evaporation}} \pm \Phi_{\text{convection}} \pm \Phi_{\text{hyporheic}}$$
    
*   $\rho_w$ = Density of water ($1,000 \text{ kg/m}^3$)
*   $C_p$ = Specific heat capacity of water ($4,186 \text{ J/(kg}\cdot^\circ\text{C)}$)
*   $Q$ = Volumetric channel discharge ($\text{m}^3\text{/s}$)
...
When the bank is shaded by *Rhododendron maximum*, the soil temperature is kept exceptionally low ($12^\circ\text{C}$ to $14^\circ\text{C}$). This preserves the cold groundwater temperature and increases the cooling capacity of the hyporheic recharge zone:

```
              THERMAL REGULATION WITHIN STEP-POOL CHANNEL
              
                Warm Sun (Unshaded Segment) -> [Solar Radiation (902 W/m²)]
                                                 |
                                                 v
  ==========================\                    |                      /=====================
        Step (Keystone)     |                    v (Water Heats)        |
           ____             |~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~|
          /    \            |                     \                     |
         /      \           |                      \ (Forced            |   Pool
        |  Rock  |__________|                       \  Hyporheic Flow)  |  [T < 58°F]
        |        |          |                        v                  | (Protected by Shading)
         \______/           \__                       \                 |
                               \                       v                |
                                \__________________\___ \               |
                                 [Gravel Bed Bed]   \    v              |
                                                     \__  \             |
                                                        \  v            |
                                                         '--------------'
                                               [Cool Bed Sub-surface Zone (13°C)]
```

By maintaining $\Phi_{\text{net\_solar}} < 40 \text{ W/m}^2$, the downstream thermal gradient $dT/dx$ remains net negative or neutral, even during late summer. This allows wild trout populations to find thermal refuge in the deep pools shaded by the evergreen canopy.
...
*   **Step-Pool Spacing ($S_p$)**: For very steep streams ($S > 0.04$), steps must be constructed using massive keystone boulders keyed into the bedrock. The step spacing is calculated as a function of channel slope:
    ...
    $$S_p = \frac{1.2 \cdot W_{bkf}}{S^{0.5}}$$
    
    Where $W_{bkf}$ is the bankfull width (ft or m) and $S$ is the channel slope (ft/ft or m/m).
*   **Pool-Riffle Spacing ($S_{pr}$)**: For moderate gradient streams ($S = 0.01 \text{ to } 0.03$), pools are spaced at:
    
    $$S_{pr} = (5.0 \text{ to } 7.0) \cdot W_{bkf}$$
...
*   **Log Vanes and J-Hook Structures**: Log structures must be constructed from rot-resistant hardwoods (Oak, Locust, or Hemlock) with a minimum diameter of $14 \text{ inches}$.
    *   **Angle of Orientation**: The logs must point upstream into the flow at an angle of $20^\circ$ to $30^\circ$ relative to the bank.
...
```
                    J-HOOK STRUCTURE PLAN VIEW
                    
                            Stream Bank (GRSL Stabilized)
                     =============================================
                              \
                               \  Log Vane Angle (20° - 30°)
                                \
                                 \
                       Flow       \       [Scour Pool Center]
                       ======>     \             _.._
                                    \           /    \
                                     \         | Pool |
                                      O---------\____/
...
*   **Assembly**: Large foundation toe rocks are placed to form a structural support frame, leaving an open cavity $1.5 \text{ ft}$ deep beneath the bankful stage.
...
```
                      CROSS-SECTION OF A TROUT SPWNING REDD
                      
                        Flow Direction (Downwelling)
                        =========================>
                         _.._
                        /    \   [Tailout Glide]
                       /      \~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ (Water Surface)
         ...
         ============/          \_________________
                                      \          /
                                       \ REDD   /  [Clean Gravel (d_50 = 15-40mm)]
                                        \______/
                                           O   O   <- [Trout Eggs Nestled in Gravel]
                                         O   O   O    (High DO > 7.0 mg/L Required)
                                       __________
                                      [Bed Clay / Bedrock Base]
```

#### 1. Target Spawning Gravel Geomorphic Specs
Restored spawning beds must conform to the following particle size distribution:
*   **$d_{50}$ (Median particle size)**: $15 \text{ mm}$ to $40 \text{ mm}$ (clean quartz or granite gravel).
*   **Fines Material Fraction ($< 2 \text{ mm}$)**: **$< 8\%$** of the total bed sample weight. High-density fines block water flow, reducing dissolved oxygen (DO) below the critical limit of $7.0 \text{ mg/L}$ and causing egg mortality.

#### 2. Hydraulic Shear Sieve Analysis & Bed Flushing Mechanics
To maintain clean gravels, the channel cross-section must be designed to generate sufficient boundary shear stress ($\tau$) during annual bankfull events to mobilize fine silts and sands ($d < 2 \text{ mm}$) while leaving the spawning gravel matrix ($d_{50} = 30 \text{ mm}$) stable.

The required boundary shear stress for flushing is calculated using a dual-Shields limit:

$$\tau_{\text{flush\_design}} = \theta_{\text{fine\_mobilize}} (\rho_s - \rho_w) g d_{\text{fine}} < \tau < \tau_{\text{gravel\_stable}} = \theta_{\text{gravel}} (\rho_s - \rho_w) g d_{50}$$
...
This stress zone is highly effective: it is high enough to mobilize and flush all accumulated fine sediments ($< 2 \text{ mm}$) downstream, yet remains well below the $21.85 \text{ N/m}^2$ structural stability limit of the spawning gravels. This self-cleaning geomorphic design ensures long-term spawning success without requiring manual maintenance.
...
```
  ...
  [Phase 1] --> Silt Curtains & Pump-Around Diversion Installed
                     |
                     v
  [Phase 2] --> Keystone Steps & Toe Foundation Rocks Placed
                     |
                     v
  [Phase 3] --> 3-Tiered GRSL Soil Lifts Assembled & Compacted
                     |
                     v
  [Phase 4] --> Mature Rhododendron Transplants Hydraulically Set
                     |
                     v
  [Phase 5] --> Live-Staking Matrix Placed & Spawning Gravel Beds Sorted
                     |
                     v
  [Phase 6] --> Pump-Around Removed & Slow Channel Rewetting Executed
```
...
*   Sift and place clean gravels ($15\text{--}40\text{ mm}$) at riffle heads and pool tailouts, maintaining a minimum depth of $1.5 \text{ ft}$ of gravel to allow redd excavation by adult spawning trout.
...
*   **Geomorphic Stability (Years 1-7)**:
    ...
    *   No structural shifting of the keystone boulders or log veins during a $10\text{-year}$ storm event.
...
*   **Riparian Vegetation Survivorship (Years 1-7)**:
    *   **$\ge 85\%$ survival rate** for transplanted *Rhododendron maximum* clusters.
    *   **$\ge 80\%$ canopy cover** achieved by Year 3, measured using hemispherical canopy photography.
    *   Minimum of **$5 \text{ stems/m}^2$** of living, established woody stakes (*Alnus* and *Cornus*) by Year 5.
*   **Thermal Regulation Success (Years 1-7)**:
    *   Water temperatures within the shaded segments must remain strictly below the maximum trout limit of **$65^\circ\text{F}$ ($18.3^\circ\text{C}$)** during peak summer monitoring (July 1 to September 1).
...
*   **Biological Recovery (Years 3, 5, and 7)**:
    *   Dissolved oxygen within spawning gravel beds must remain **$\ge 7.0 \text{ mg/L}$** throughout the winter incubation phase (November to March).
    *   Macroinvertebrate communities must display a positive trend, with EPT (Ephemeroptera, Plecoptera, Trichoptera) richness scores increasing by **$\ge 35\%$** compared to pre-restoration baselines.
